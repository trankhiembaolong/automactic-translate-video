...
async def create_audio(subtitle_file, output_folder, combined_output):
    ...
    # Sử dụng semaphore để giới hạn kết nối đồng thời
    SEMAPHORE_LIMIT = 10  # Số kết nối đồng thời tối đa
    semaphore = asyncio.Semaphore(SEMAPHORE_LIMIT)

    async def limited_tts_request(communicate, tts_file):
        async with semaphore:
            try:
                await communicate.save(tts_file)
            except Exception as e:
                logging.error(f"Lỗi tạo TTS cho {tts_file}: {e}")

    # Tạo TTS cho tất cả phụ đề
    tasks = []
    for sub in subtitles:
        try:
            ...
            if not tts_file.exists():
                ...
                tasks.append(limited_tts_request(communicate, str(tts_file)))
        except Exception as e:
    ...
    # Điều chỉnh tốc độ âm thanh song song
    process_audio_parallel(subtitles, output_folder)

    # Ghép file âm thanh cuối cùng
